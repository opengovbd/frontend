...
##### Use `$govuk-output-custom-properties` to control use of custom properties in your CSS
...
The latest palette was initially opt-in, using the `$govuk-new-organisation-colours` feature flag. The updated palette is now the only palette available, so you must remove the feature flag from your code.
